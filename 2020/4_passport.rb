...
ECLS = %w(amb blu brn gry grn hzl oth).to_set
valids = passports.find_all do |passport|
    case passport
    in {
        byr: "1920".."2002",
        iyr: "2010".."2020",
        ...
        ecl: ECLS,
        pid: /\A[0-9]{9}\Z/,
    } if (hgt =~ /cm/ ? 150..193 : 59..76).include?(hgt.to_i)
        true
    else
        false
    end
    # required & p.keys == required
    # ("1920".."2002").include?(p["byr"]) &&
    # ("2010".."2020").include?(p["iyr"]) &&
    # ("2020".."2030").include?(p["eyr"]) &&
    # (hgt = %r'\A(\d+)(cm|in)\Z'.match(p["hgt"])) &&
    # (hgt[2] == 'cm' ? ("150".."193") : ("59".."76")).include?(hgt[1]) &&
    # %r'\A#[0-9a-f]{6}\Z'.match(p["hcl"]) &&
    # %w(amb blu brn gry grn hzl oth).include?(p["ecl"]) &&
    # %r'\A[0-9]{9}\Z'.match(p["pid"])
end
p passports.size
p valids.size
