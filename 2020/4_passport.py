...
def main():
    data = FULL
    passport_strs = data.split("\n\n")
    def parse(pstr):
        fields = re.split(r"[\n ]", pstr)
        return dict(field.split(":", 2) for field in fields)
    passports = [parse(p) for p in passport_strs]
    allfields = [p for p in passports if REQUIRED & p.keys() == REQUIRED]
    print(len(allfields))

    def validate(p: dict):
        def valid_height(hgt):
            parts = re.split(r"(?=cm|in)", hgt, 1)
            if len(parts) == 2:
                num, unit = parts
                if unit == "cm":
                    return int(num) in range(150, 194)
                elif unit == "in":
                    return int(num) in range(59, 77)
            return False
        return int(p['byr']) in range(1920, 2003) and \
               int(p['iyr']) in range(2010, 2021) and \
               int(p['eyr']) in range(2020, 2031) and \
               valid_height(p['hgt']) and \
               re.fullmatch(r"#[0-9a-f]{6}", p['hcl']) and \
               p['ecl'] in {"amb", "blu", "brn", "gry", "grn", "hzl", "oth"} and \
               re.fullmatch(r"[0-9]{9}", p['pid'])

    valid = list(filter(validate, allfields))
    print(len(valid))
