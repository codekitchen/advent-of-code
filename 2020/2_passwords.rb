input = DATA
parsed = input.each_line.map do |line|
    raw_policy, password = line.split(": ")
    range, letter = raw_policy.split(" ")
    low, high = range.split("-").map(&:to_i)
    [[low, high, letter], password]
end
valid = parsed.filter do |(low, high, letter), password|
    count = password.split(//).filter { |c| c == letter }.size
    (low..high).include?(count)
end
puts valid.size

valid2 = parsed.filter do |(i1, i2, letter), password|
    v1 = password[i1-1] == letter
    v2 = password[i2-1] == letter
    (v1 || v2) && !(v1 && v2)
end
p valid2.size
