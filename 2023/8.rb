...
def part1(input)
  steps, locs = parser(input)
  loc = "AAA"
  steps.each_with_index do|s,n|
    loc = locs[loc][s == "L" ? 0 : 1]
    return n+1 if loc == "ZZZ"
  end
end

def part2(input)
  steps, locs = parser(input)
  currents = locs.keys.select { _1.end_with?("A") }
  lcm = [1] * currents.length
  steps.each_with_index do |s,n|
    currents = currents.map { |loc| locs[loc][s == "L" ? 0 : 1] }
    return n+1 if currents.all? { _1.end_with?("Z") }
    currents.each_with_index do |loc,t|
      if loc.end_with?("Z") && lcm[t] == 1
        lcm[t] = n+1
        return lcm.reduce(1, :lcm) if lcm.all? { _1 > 1 }
      end
    end
  end
end
...
puts "part1 short", (part1 SHORT1).inspect
puts "part1 full", (part1 FULL).inspect
puts "part2 short", (part2 SHORT2).inspect
