...
Round = Data.define(:red, :green, :blue) do
  def self.parse(str)
    parts = str.split(",")
    counts = {"red" => 0, "blue" => 0, "green" => 0}
    parts.map(&:split).each { |n,color| counts[color] = n.to_i }
    self.new(**counts)
  end
...
assert_eq Round.parse(" 5 blue, 4 red, 13 green"), Round.new(4, 13, 5)

Game = Data.define(:id, :rounds) do
  def self.parse(line)
    hdr, rest = line.split(":", 2)
    id = hdr.split(" ").last.to_i
    rounds = rest.split(";").map { Round.parse(_1) }
    self.new(id, rounds)
  end
end
assert_eq Game.parse("Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green"), Game.new(5, [
  Round.new(6, 3, 1),
  Round.new(1, 2, 2),
])

def part1(input)
  games = input.lines.map { Game.parse _1 }
  valid = games.select { |g| g.rounds.all? { |r| r.valid?(12, 13, 14) } }
  valid.sum { _1.id }
end

def part2(input)
  games = input.lines.map { Game.parse _1 }
  games.sum { |g| g.rounds.map(&:red).max * g.rounds.map(&:green).max * g.rounds.map(&:blue).max }
end
...
SHORT = %{Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
