...
def findit(line, it)
  it.each { |i| VALS.each { |k,v| return v if line[i..].start_with?(k) || line[i..].start_with?(v.to_s) } }
...
def part2(input)
  input.lines.sum do |line|
    first = findit(line, 0.upto(line.length))
    last = findit(line, line.length.downto(0))
    "#{first}#{last}".to_i
  end
end
