...
def symin?(line, a, b)
  a = 0 if a < 0
  line && line[a..b].chars.any? { sym?(_1) }
end
  ...
  0.upto(grid.length-1) do |y|
    ...
    while x < line.length
      ...
      if !start && digit?(c)
        ...
        num = line[start...x].to_i
        if sym?(line[start-1]) ||
              sym?(line[x]) ||
              (y>0 && symin?(grid[y-1], start-1, x)) ||
              symin?(grid[y+1], start-1, x)
          sum += num
        end
        start = nil
      end
...
Num = Data.define(:y, :x1, :x2, :val) do
  def touching?(oy, ox)
    ((x1-1)..(x2+1)).include?(ox) &&
      ((y-1)..(y+1)).include?(oy)
  end
end
  ...
  grid.each_with_index do |line,y|
    ...
    line.chars.each_with_index do |c,x|
      if !start && digit?(c)
        ...
        num = line[start...x].to_i
        nums << Num.new(y, start, x-1, num)
        start = nil
      end
    end
  end

  # found all the nums, now find gears
  sum = 0
  grid.each_with_index do |line,y|
    line.chars.each_with_index do |c,x|
      if c == '*'
        adj = nums.select { _1.touching?(y, x) }
        if adj.length == 2
          ratio = adj[0].val * adj[1].val
          sum += ratio
        end
      end
    end
