...
def part1(input, start_shape)
  grid = {}
  input.each_line.with_index { |l,y| l.strip.chars.each_with_index { |c,x| grid[P[x,y]] = c } }
  start = grid.key 'S'
  grid[start] = start_shape

  q = [[start, 0]]
  distances = {}
  until q.empty?
    n, d = q.shift
    next unless grid.key?(n) and !distances.key?(n)
    distances[n] = d
    ADJ[grid[n]].each { q << [n+_1, d+1] }
  end
  distances.each_value.max
end
...
def part2(input, start_shape)
  ...
  (0..).each do |d|
    cur, q = q, []
    cur.each { |n| distances[n] ||= d; q += n.connected.select { !distances.key?(_1) } }
    break if q.empty?
  end
...
# puts "part2 full", (part2 FULL, ?L).inspect
